...
void draw(cv::Mat& img, const std::vector<face_pipeline::trt::FaceDetection>& faces) {
    for (const auto& f : faces) {
        ...
        const std::array<cv::Scalar, 5> lm_colors{
        ...
        cv::putText(img, buf,
                    cv::Point(static_cast<int>(f.bbox.x),
                              std::max(0, static_cast<int>(f.bbox.y) - 5)),
...
int main(int argc, char** argv) {
    ...
    try {
        ...
        if (is_image(input_path)) {
            ...
            if (img.empty()) throw std::runtime_error("cannot read image: " + input_path);
